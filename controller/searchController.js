...
exports.selectedClub = async (req, res) => {
  ...
  try {
    ...
    var clubImageQuery = querie.queryList.CLUB_IMAGE;
    var clubImage = await dbconnection.dbQuery(clubImageQuery, [clubProfileDB.rows[0].image_id]);
    var from = parseInt(
    ...
    var image=[];
    clubImage.rows.forEach(e => {
      image.push(e[`url_image1`]);
      image.push(e[`url_image2`]);
      image.push(e[`url_image3`]);
      image.push(e[`url_image4`]);
    });
    console.log(image);
    //console.log(date);
    return res.status(200).json({
      ...
      clubImage: /* clubImage.rows */image,
